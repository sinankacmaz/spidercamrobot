...
void Esp32Cam_Data_Yolla(int a,int b){
 int x = 16;
 int m = b * x;
 m = m + a;
 Serial.write(m);
...
void Esp32Cam_Data_Alma(){
Esp32CamData=' ';
    if(Serial.available()>0){
    Esp32CamData = Serial.read();
    //Serial.print("Esp32CamData:"); Serial.println(char(Esp32CamData));
    switch(Esp32CamData){
      case 'Z':
      //Forward
      step_forward(5);
...
x_ekseni_deger = map(accx , -17000 , 17000 , 0 , 12);
y_ekseni_deger = map(accy , -17000 , 17000 , 0 , 12);
    
Esp32Cam_Data_Yolla(x_ekseni_deger , y_ekseni_deger);
